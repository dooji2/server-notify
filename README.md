...
All commands have some auto-suggestion, therefore I will not go over all of them as they are pretty straightforward.

To create a new notification, all types have a base command:
```
/server-notify new "Notification Name" type type
```
Both **type** should be the same, they can be **texture**, **text** or **url** (for now).
...
```
/server-notify new "Notification Name" texture texture <sound_namespace> <sound_texture_path> <texture_namespace> <texture_path> <width> <height> <dismissMessage> <alwaysShow>
```
...
**<sound_texture_path>** is the path of the sound you'd like to play when the notification is shown.
...
**<alwaysShow>** shows the notification for a player when they join even if they have already seen it.
</details>
...
An example command would be:
```
/server-notify new "Notification Name" text text <sound_namespace> <sound_texture_path> <message> <dismissButton> <dismissMessage> <alwaysShow>
```
...
An example command would be:
```
/server-notify new "Notification Name" url url <sound_namespace> <sound_texture_path> <url> <width> <height> <dismissMessage> <alwaysShow>
```
